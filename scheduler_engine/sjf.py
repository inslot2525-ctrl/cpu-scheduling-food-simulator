...
Orders with shortest preparation time are processed first.
"""
...
class SJFScheduler:
    ...
    def __init__(self, preemptive: bool = False):
        ...
        self.name = "Shortest Remaining Time First (SRTF)" if preemptive else "Shortest Job First (SJF)"
    
    def schedule(self, orders: List[Order]) -> Tuple[List[Order], List[Tuple[int, int, int]]]:
        """
        Schedule orders using SJF/SRTF algorithm.
        ...
        while len(completed) < len(work_orders):
            ...
            if not ready_queue:
                # No orders ready, advance time
                current_time += 1
                continue
            
            # Select order with shortest remaining time
...
def run_sjf(orders: List[Order], preemptive: bool = False) -> Tuple[List[Order], List[Tuple[int, int, int]]]:
    """Convenience function to run SJF scheduling."""
